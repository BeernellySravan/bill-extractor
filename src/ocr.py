...
def load_document_images(document_path, dpi=300):
    ...
    if str(document_path).lower().startswith("http://") or str(document_path).lower().startswith("https://"):
...
def run_ocr_on_image(image):
    """
    Run Tesseract OCR on a PIL Image and return the detailed word boxes dict.
    """
